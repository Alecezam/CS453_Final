...
// return the edge with the supplied vertices at its endpoints
...
// find what quad the supplied coordinates are in
Quad* Polyhedron::find_quad(double x, double y)
{
	for (int i = 0; i < nquads; i++)
	{
		Quad* quad = qlist[i];
		double x1 = quad->verts[0]->x, y1 = quad->verts[0]->y, x2 = quad->verts[0]->x, y2 = quad->verts[0]->y;
		// Find x1, x2, y1, y2
		...
		for (int j = 0; j < 4; j++) {
			if (x1 > quad->verts[j]->x) {
				x1 = quad->verts[j]->x;
			}
			if (x2 < quad->verts[j]->x) {
				x2 = quad->verts[j]->x;
			}
			if (y1 > quad->verts[j]->y) {
				y1 = quad->verts[j]->y;
			}
			if (y2 < quad->verts[j]->y) {
				y2 = quad->verts[j]->y;
			}
		}
